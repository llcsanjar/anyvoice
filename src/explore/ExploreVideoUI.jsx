// src/explore/ExploreVideoUI.jsx
...
const ExploreVideoUI = ({
  ...
  const createPlaceholders = (count) => {
    ...
    for (let i = 0; i < count; i++) {
      items.push(
        <div className="video-placeholder" key={`placeholder-${i}`}>
  ...
  const loadMoreVideos = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`${backendUrl}/get-random-videos`, {
        params: {
          limit,
          user_id: userId,
      ...
      message.error(t('exploreVideo.error'));
      clearPlaceholders();
    } finally {
      setLoading(false);
        ...
        {videos.map(video => (
          ...
          >
            <VideoLoader
              ...
              fullContainerRef={fullContainerRef} // Интиқоли ref ба VideoLoader
            />
          </div>
        ))}
